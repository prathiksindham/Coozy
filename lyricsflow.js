...
const LyricsFlow = (() => {
  ...
  const TEXT_DISTANCE = 4;        // gap between the body edge and the text ring, px
  ...
  function toScreen(pts) {
    ...
    const bottomLimit = H - 8 - GLYPH_H / 2 - TEXT_DISTANCE;
    for (let i = 0; i < pts.length; i += 2) {
      const u = (pts[i] - 0.5) / MASK, v = (pts[i + 1] - 0.5) / MASK;
      pts[i] = ox + vw * (1 - u) * scale;
      let py = oy + vh * v * scale;
      if (py > bottomLimit) py = bottomLimit;
      pts[i + 1] = py;
    }
  }
  ...
  async function loadLyrics() {
    ...
    // Wait up to 3s for the audio duration to become available so the server
    // can match the correct version of the song's synced lyrics.
    let dur = 0;
    try { dur = Math.round(curDuration()) || 0; } catch (_) {}
    if (!dur) {
      for (let waited = 0; waited < 400; waited += 100) {
        await new Promise((r) => setTimeout(r, 100));
        if (seq !== lyrSeq || !isOpen) return;
        try { dur = Math.round(curDuration()) || 0; } catch (_) {}
        if (dur) break;
      }
    }
    ...
    if (j && j.synced && j.synced.length) {
      lyrLines = j.synced.map((r) => ({ t: r.t, text: _cleanTxt(r.text) })).filter((l) => l.text && _isClean(l.text));
    }
    if ((!lyrLines || !lyrLines.length) && j && j.plain) {
      const raw = String(j.plain).split(/\r?\n/).map(l => _cleanTxt(l)).filter(l => l && _isClean(l));
      if (raw.length) {
        const dSec = (dur && dur > 20) ? dur : 180;
        const startT = 6.0, endT = Math.max(startT + 10, dSec - 10);
        const step = raw.length > 1 ? (endT - startT) / (raw.length - 1) : 0;
        lyrLines = raw.map((text, i) => ({ t: Math.round((startT + i * step) * 100) / 100, text }));
      }
    }
    if (lyrLines && lyrLines.length) {
      lyrMode = "synced";
      curLine = -1;
  ...
  window.syncLyricsFlowSeek = function() {
    if (lyrMode === "synced") {
      const cur = currentLineIdx();
      curLine = cur;
      bakeWindow(cur);
  ...
  function placeQuads(out) {
    ...
    for (let c = 0; c < copies && n < t.maxQuads; c++) {
      for (let k = 0; k < stripChars.length && n < t.maxQuads; k++) {
        ...
        if (s + wpx > pathLen) continue;
        const sMid = s + wpx / 2;
        samplePath((sMid + 10) % pathLen, _sB);
        samplePath((sMid - 10 + pathLen) % pathLen, _sA);
        let ang = Math.atan2(_sB.y - _sA.y, _sB.x - _sA.x);
        // Bottom half: flip 180° so the text reads upright
  ...
  async function open() {
    if (isOpen) return;
    for (const other of [window.FallingHearts, window.FallingSnow, window.FallingBubbles, window.KineticAscii]) {
